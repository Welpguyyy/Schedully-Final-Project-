...
{
    public interface IfirstControl
    {
        void load();
    ...
    public partial class UserControlFirstaid : UserControl, IfirstControl
    {
        int count = 0;
        public UserControlFirstaid()
        {
            InitializeComponent();
        }

        private void UserControlFirstaid_Load(object sender, EventArgs e)
        {
            load();
        }
        public void load()
        {
            using (OleDbConnection conn = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\\Users\\zhient\\Desktop\\projects\\Schedully\\bin\\Debug\\Schedully.accdb"))
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT FirstAid FROM Medicine_Quantity"; // Fetch all rows from Cold column
                    ...
                    using (OleDbCommand cmd = new OleDbCommand(query, conn))
                    {
                        using (OleDbDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            {
                                quantities.Add(reader["FirstAid"].ToString());
                            }

                            // Check if there are enough values before accessing them
                            if (quantities.Count >= 5)
                            {
                                thermolbl.Text = "X" + quantities[0];
                                gauzelbl.Text = "X" + quantities[1];
                                bandlbl.Text = "X" + quantities[2];
                                bplbl.Text = "X" + quantities[3];
                                gloveslbl.Text = "X" + quantities[4];
        ...
        public int GetPillID(string Name)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\\Users\\zhient\\Desktop\\projects\\Schedully\\bin\\Debug\\Schedully.accdb"))
            {
                string query = "SELECT PillID FROM Medicine_names WHERE FirstAid = @Name";
                using (OleDbCommand cmd = new OleDbCommand(query, conn))
                {
        ...
        public void Update(int pillID, string newValue)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    string query = "UPDATE Medicine_Quantity SET FirstAid = @New WHERE PillID = @PillID";
        ...
        public int GetPillvalue(int pillid)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=C:\\Users\\zhient\\Desktop\\projects\\Schedully\\bin\\Debug\\Schedully.accdb"))
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT FirstAid FROM Medicine_Quantity";
                    ...
                    using (OleDbCommand cmd = new OleDbCommand(query, conn))
                    {
                        using (OleDbDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                if (int.TryParse(reader["FirstAid"].ToString(), out int woundValue))
                                {
        ...
        private void addbtn_Click(object sender, EventArgs e)
        {
            ...
            firstlbl.Text = Convert.ToString(count);
        }
        ...
        {
            string selected = firstbox.SelectedItem.ToString();
